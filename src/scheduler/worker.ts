...
2. ${isWebflow
    ? `ONE body-copy strengthening from the bank's copy_optimise entries, plus incidental fixes only on pages you already read this run. Do NOT file blog-meta or internal-link actions from the bank — the daily bank_drain cycle ships those at volume and you would duplicate or churn-cap-block its work.`
    : `Up to ${typeof tenant.dailyActionCap === 'number' && tenant.dailyActionCap > 0 ? Math.max(1, tenant.dailyActionCap - 1) : '4-6'} on-page improvements for existing pages — QUALITY FLOOR, not quota: every action must be grounded in data you read this run and pass the server-side gates. If the bank only supports fewer strong actions today, file fewer. Never manufacture a mediocre edit to hit a count; padded actions get rejected by the critic and waste the run.`}${typeof tenant.dailyActionCap === 'number' && tenant.dailyActionCap > 0 ? ` HARD CAP for this tenant: ${tenant.dailyActionCap} propose_action calls total this run, article included.` : ''} Pick the right tool based on what you're changing:
${cmsToolList}
